...
def update_cp(id_cp:int,cp:Cp_update,session : Session):
    try:
    ...
    try:
        chargepoints = session.exec(
            select(
                ...
                func.array_agg(Connector.status).label("status_connectors")
            )
        ...
        formatted_result = [
            {
                ...
                "adresse": cp.adresse,
                "connecteurs": [
                    {"status": status} for status in cp.status_connectors
                ],
