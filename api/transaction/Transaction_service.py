...
from sqlmodel import Session as Session_db, case

from api.CP.CP_services import send_remoteStopTransaction
...
from api.users.UserServices import get_user_by_id_trans
from core.utils import UNIT_KWH, UNIT_WH
from models.Pagination import Data_display
from models.elecdis_model import Session as SessionModel, Historique_metter_value, Transaction, Historique_session
...
def create_session_service(session: Session_db, session_data: Session_create):
    ...
    ts = create_new_tarif_snapshot(session_model.id, session_model.start_time, session_model.metter_start / 1000,
                                   session, None)
    session.commit()
...
def update_session_service_on_stopTransaction(session: Session_db, session_data: Session_update):
    session_model: SessionModel = session.exec(
        select(SessionModel).where(SessionModel.id == session_data.transaction_id)).first()
...
def count_current_session(session: Session_db):
    count = session.exec(
        select(func.count(SessionModel.id)).where(SessionModel.end_time == None).where(SessionModel.id != -1)).one()
    return count
...
def get_session_data_2(session: SessionModel, session_db: Session_db):
    ...
    if session is not None and user is not None:
        data = Session_data_affichage(
            ...
            start_time=session.start_time,
            end_time=session.end_time,
            connector_id=session.connector_id[0] if session.connector_id is not None else None,
            user_id=session.user_id,
...
def get_transactions_details_by_session(session_db: Session_db, session_id: int, page: int, number_items: int):
    ...
    count = session_db.exec(select(func.count(Transaction.id)).where(Transaction.session_id == session_id).where(
        SessionModel.id != -1)).one()
    transactions = session_db.exec(
        select(Transaction).where(Transaction.session_id == session_id).where(SessionModel.id != -1).offset(
            paginations.offset).limit(paginations.limit)).all()
...
def get_transactions_by_user_id(user_id: int, session: Session_db, page: int, number_items: int):
    ...
    transactions = session.exec(
        select(SessionModel).where(SessionModel.user_id == user_id).offset(paginations.offset).limit(
            paginations.limit)).all()
    paginations.total_items = count
...
def get_transactions_by_user_tags(user_tag: str, session: Session_db, page: int, number_items: int):
    ...
    print("hehe")
    transactions = session.exec(
        select(SessionModel).where(SessionModel.tag == user_tag).offset(paginations.offset).limit(
            paginations.limit)).all()
    paginations.total_items = count
    datas = Data_display(data=get_list_session_data_2(transactions, session_db=session), pagination=paginations)
    return datas
...
def search_transactions_by_date(session: Session_db, date_start: date, date_end: date, montant_fin, montant_debut,
                                energy_fin, energy_debut, page, number_items):
    ...
    if montant_debut is not None and montant_fin is not None:
        ...
        query = (
            ...
            .having(func.sum(Transaction.total_price) >= montant_debut,
                    func.sum(Transaction.total_price) <= montant_fin)
        )
...
def search_transaction_by_date_v3(state:str, session: Session_db, date_start: date, date_end: date, montant_fin, montant_debut,
                                  energy_fin, energy_debut, page, number_items):
    from api.Historique_session.Historique_session_services import get_lists_historique_datas
    subquery = (
        select(
            Historique_session.id,
            func.coalesce(func.sum(
                case(
                    (Historique_session.is_credit == True, Transaction.consumed_energy_added),
                    else_=Transaction.consumed_energy
                )
            ),0).label("total_energy"),
            func.coalesce(func.sum(Transaction.total_price),0).label("totalPrice")
        )
        .join(SessionModel, SessionModel.id_historique_session == Historique_session.id)
        .join(Transaction, Transaction.session_id == SessionModel.id, isouter=True)
        .group_by(Historique_session.id)
        .subquery()

    )
    # Requête principale pour joindre avec `HistoriqueSession`
    query = (
        select(Historique_session)
        .join(subquery, subquery.c.id == Historique_session.id)
    )
    if (date_start is not None and date_end is not None):
        if date_start > date_end:
            raise Exception("La date initiale doit être antérieure à la date de finale")
        query = query.where(func.date(Historique_session.start_time).between(date_start, date_end))
    # montants
    if montant_debut is not None and montant_fin is not None:
        if montant_debut > montant_fin:
            raise Exception("Le montant initial doit être inférieur au montant de final")
        query = query.where(subquery.c.totalPrice.between(montant_debut, montant_fin))
    if energy_debut is not None and energy_fin is not None:
        if energy_debut > energy_fin:
            raise Exception("L'energie initial doit être inférieur à l'energie de final")
        query = query.where(subquery.c.total_energy.between(energy_debut, energy_fin))
    if state is not None:
        if state.strip() == "terminé":
            query = query.where(Historique_session.state != 1)
        elif state.strip()== "en cours":
            query = query.where(Historique_session.state == 1)

    query = query.order_by(desc(Historique_session.start_time))
    count_query = select(func.count()).select_from(
            query.subquery()
        )
    totalItem = session.exec(count_query).one()
    pagination = Pagination(page=page, limit=number_items)
    pagination.total_items=totalItem
    result = get_lists_historique_datas(session.exec(query.offset(pagination.offset).limit(pagination.limit)).all(),session)
    pg=pagination.dict()
    return{"data":result,"pagination":pg}




def search_transactions_by_date2(session: Session_db, date_start: date, date_end: date, montant_fin, montant_debut,
                                 energy_fin, energy_debut, page, number_items):
    query = select(SessionModel).where(SessionModel.id != -1)
    query_count = select(func.count(SessionModel.id))
...
async def stop_transactions_on_sold_out(session: Session_db, idtag: int, session_id: int, metervalue: float,
                                        charge_point_id):
